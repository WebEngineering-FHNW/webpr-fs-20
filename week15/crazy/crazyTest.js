...
crazySuite.add("equals", assert => {
    ...
    assert.is( a == b, true);
    assert.is( b == c, true);
    assert.is( a == c, false); // broken transitivity of equals
});

crazySuite.add("false", assert => {

    assert.true( ! false     );  // in HTML
    assert.true( ! null      );  // <p > <p disabled>  <p disabled="">
...
crazySuite.add("coercion", assert => {
    ...
    // assert.is  ( new Number("0"), 0 ); // do not use "new" !, see below
});

crazySuite.add("object", assert => {

    const conditioned = x => x ? true : false ;

    assert.is( conditioned("0")            , true);
    assert.is( conditioned(+"0")           , false);
    assert.is( conditioned(Number("0"))    , false);
    assert.is( conditioned(new Number("0")), true);
...
crazySuite.add("other", assert => {
    ...
    assert.is( {} + [],      "[object Object]"); // is 0 in console
